...
1. CPU 按遍历顺序生成种子。若条件以出生点为锚点，多个工作线程先调用较便宜的 `estimateSpawn`。
2. 当前现代出生算法保证估计点到最终点不超过 125 格；GPU 用相应扩大后的半径做第一轮保守粗筛。下界投影会把这个上界安全换算为 18 格。
3. CPU 只为粗筛幸存者计算带地表落脚检查的完整出生点，GPU 再用原始半径精筛；固定坐标条件不需要这一级出生点准备。
4. 最终幸存种子回到原有 Cubiomes 路径，完整检查生物群系、地形、结构竞争、Jigsaw 布局、末地要塞眼数等条件。
5. 只有 CPU 精确路径通过的种子才会输出；完整出生点会在报告阶段复用，不会再次生成。
...
同一查询的粗筛和精筛会分别按各自半径与结构分区面积估算放置密度，让更可能淘汰候选的条件先执行并尽早结束当前线程；这只重排必须同时成立的条件，不影响筛选语义。

`--threads` 在 GPU 模式下仍控制 CPU 出生点估算、幸存者细化和精确复核并行度，通常保留系统默认值即可。`--batch-size` 默认 4096；更大的批次可提高很便宜的固定坐标结构筛选吞吐量，但会拉长进度刷新和找到足够结果后的停止延迟。优先从 4096 开始测量，再按实际查询调整。进度与完成摘要会分别显示出生点粗筛和完整 GPU 预筛选的保留数/原始候选数，便于判断查询是否具有足够选择性。
