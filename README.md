...
# 首次击杀末影龙后，穿过第一座折跃门到达的外岛出口 500 格内有末地城
mcseed-finder find --structure-near end_city:500
```
...
- `--structure-near STRUCTURE[,STRUCTURE]:RADIUS`：结构自身决定维度；主世界默认以出生点为中心，下界默认以 `floor(spawn / 8)` 为中心，末地默认以首次击杀末影龙后第一座折跃门对应的外岛出口为中心。
...
- `--batch-size` 控制 GPU 批处理、进度刷新和停止检查粒度，默认 4096。出生点管线会缓存外层候选的噪声状态，因此超大批次会明显增加显存占用且不保证更快；建议用固定种子短测 10000、20000、40000 等档位。10 GB RX 6750 GRE 上，本项目的重出生点组合实测约 40000 最快，`120000` 略慢且停止延迟更大。
...
`anchor` 可以是 `origin`、`spawn`、`nether_spawn`、`end_gateway`，也可以是自定义坐标，例如 `{"x": 1200, "z": -300}`。`end_gateway` 表示首次击杀末影龙后生成的第一座主岛折跃门所链接的外岛出口；兼容别名是 `end_gateway_exit`。末地条件省略 `anchor` 时默认使用它，显式写 `origin` 可恢复以 `(0, 0)` 为中心的查询。用 `min_count: 0, max_count: 0` 可以表达“范围内不能有该目标”。配置采用严格字段校验，拼错的字段会直接报错。
...
- `end_gateway` 会复现世界种子决定的 20 座主岛折跃门顺序，选择首次屠龙后生成的第一座，并按末地岛屿与地表噪声寻找其链接的外岛折跃门坐标；若向外搜索完整 16 步仍没有可用区块，也会复现原版生成临时末地岛的回退路径。比较使用外岛折跃门的水平坐标；后续复活末影龙生成的另外 19 座门不参与这个锚点。
...
GPU 预筛选覆盖注册表中采用随机分区放置的 20 个结构族，并额外覆盖现代版本的同心环要塞；废弃矿井继续由 CPU 处理。需要出生点的查询采用分级管线：先用严格原点包络筛结构，再以 64 线程核求外层出生点候选，以 649/84 格版本上界和同锚跨维几何关系淘汰种子，最后只为幸存者执行完整内层估算。它不会把出生点不安全地假定为 `(0, 0)`。要塞 GPU 阶段只计算原版近似环，并以生物群系搜索半径和设备数学舍入余量扩张包络，幸存者仍由 Cubiomes 精确定位。末地折跃门锚点会在 GPU 上复现种子决定的第一座门方向，并以版本配置给出的严格误差包络扩大结构半径；CPU 最终仍使用真实外岛地形出口。程序会自动提取必须成立的正向 `structure_near` 条件、正向 `structure_piece_near` 所必需的父结构候选，以及共享同一出生点锚点的随机分区结构对所必须满足的共址关系。`any`、`not`、纯上限/排除条件不会被不安全地下推；不能加速的条件始终留在 CPU 精确路径中。
